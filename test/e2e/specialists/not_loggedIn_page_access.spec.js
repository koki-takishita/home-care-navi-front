describe('未ログインのケアマネがアクセスできる画面', () => {
  ...
  it('新規登録画面をエラーなくレンダリングできている', async () => {
    ...
    await expect(url).toEqual('http://127.0.0.1:9000/specialists/users/new')
  ...
  it('ログイン画面をエラーなくレンダリングできている', async () => {
    ...
    const ele = await page.$('h6')
    const titleText = await page.evaluate((elm) => elm.textContent, ele)
    await expect(url).toEqual('http://127.0.0.1:9000/specialists/login')
  ...
  it('プライバシーポリシー画面をエラーなくレンダリングできている', async () => {
    ...
    await expect(url).toEqual(
      'http://127.0.0.1:9000/specialists/privacy_policy'
    )
  ...
  it('利用規約画面をエラーなくレンダリングできている', async () => {
    ...
    const ele = await page.$('h4')
    const titleText = await page.evaluate((elm) => elm.textContent, ele)
    await expect(url).toEqual('http://127.0.0.1:9000/specialists/terms')
  ...
  it('お問い合わせ画面をエラーなくレンダリングできている', async () => {
    ...
    const ele = await page.$('h4')
    const titleText = await page.evaluate((elm) => elm.textContent, ele)
    await expect(url).toEqual('http://127.0.0.1:9000/specialists/contacts/new')
